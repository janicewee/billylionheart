...
import { db } from '@/db';
import { reviews, discussions, userPoints } from '@/db/schema';
...
export async function POST() {
  try {
    ...
    // Get all reviews and discussions
    ...
    for (const [userId, stats] of userStats.entries()) {
      ...
      if (existing.length > 0) {
        ...
        results.push({ userId, action: 'updated', totalPoints, stats });
      } else {
        // Insert new
        ...
        results.push({ userId, action: 'created', totalPoints, stats });
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'User points backfilled successfully',
      processedUsers: userStats.size,
      results
    });
  } catch (error) {
